                ...
                case "btnOk":
                    if (MapLayer?.LayerType == "ShapefileClass")
                    {
                        ...
                        if (sfSource?.ShapefileType == ShpfileType.SHP_POINT)
                        {
                            ...
                            var sfConverted = ShapefileLayerHelper.ConvertToGrid25(sfSource, global.MainForm.UTMZone, inlandAction, includeCoordinates: chkIncludeCoordinates.Checked);
                            var name = Path.GetFileNameWithoutExtension(MapLayer.FileName);
                            if (global.MainForm.MapLayersHandler.AddLayer(sfConverted, name + "_converted") >= 0)
                            {
                                Close();
                            }
                            else
