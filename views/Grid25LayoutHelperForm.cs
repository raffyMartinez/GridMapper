...
namespace GridMapper.views
{
    ...
    public partial class Grid25LayoutHelperForm : Form
    {
        ...
        public static Grid25LayoutHelperForm GetInstance(Grid25MajorGrid majorGrid, MainForm parentForm)
        {
        ...
        public Grid25LayoutHelperForm(Grid25MajorGrid majorGrid, MainForm parentForm)
        {
            InitializeComponent();
            ...
            if (!layerFormIsOpen)
            {
                ...
                switch (global.MappingMode)
                {
                    case fad3MappingMode.grid25Mode:
                        _majorGrid.LockMap();
                        //_majorGrid.LayoutMap = chkLayout.Checked;
        ...
        private bool BatchExportMain2(GridMapSideToPrint sideToPrint)
        {
            ...
            SaveMapImage smi = new SaveMapImage(fileName, _exportDPI, global.MainForm.MapControl);
            smi.SaveToLayout = chkLayout.Checked;
            if(chkLayout.Checked)
            {
                smi.LogoScaleFactor = double.Parse(txtLogoScaleFactor.Text);
            }
            smi.MaintainOnePointLineWidth = true;
            smi.MapLayersHandler = global.MainForm.MapLayersHandler;
        ...
        private void OnButtonClick(object sender, EventArgs e)
        {
            switch (((Button)(sender)).Name)
            {
                ...
                case "btnInputTitles":
                    ...
                    EditShapeAttributeForm esaf = EditShapeAttributeForm.GetInstance(global.MainForm, _majorGrid.MapInterActionHandler);
                    if (esaf.Visible)
                    {
                        esaf.BringToFront();
                    }
        ...
        private void OnFormLoad(object sender, EventArgs e)
        {
            ...
            txtLogoScaleFactor.Enabled = false;
            if (global.MappingMode == fad3MappingMode.grid25Mode)
            {
                ...
                txtLogoScaleFactor.Text = "0.5";
            }
            else
            {
                _grid25GeographicDisplayHelper = new Grid25GeographicDisplayHelper(global.MainForm.MapLayersHandler, global.MainForm.MapControl);
                ReadLayoutData();
                var layoutShapeFileName = _layoutFileName.Replace(".lay", ".shp");

                var result = global.MainForm.MapLayersHandler.FileOpenHandler(layoutShapeFileName, "Layout frame", reproject: true);
                if (!result.success)
                {
                ...
                {
                    _layoutShapefile = (Shapefile)global.MainForm.MapLayersHandler.get_MapLayer("Layout frame").LayerObject;
                    ...
                    global.MainForm.MapControl.Extents = _layoutShapefile.Extents;
        ...
        private void OnFormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (_layoutShapefile != null)
            {
                ...
                global.MainForm.MapLayersHandler.RemoveLayer("Layout frame");
            }
            ...
            global.MainForm.SetCursor(tkCursorMode.cmSelection);
            ...
            else
            {
                ...
                if (isVisible)
                {
                        ...
                        if (chkAutoExpand.Checked)
                        {
                            ...
                            if (_grid25GeographicDisplayHelper.MaxDimensionMBR != null)
                            {
                                ...
                                ext.SetBounds(extMBRGrid.xMin - (labelDistance * 3) - XBuffer / adjustValue,
                                    ...
                                    (extMBRGrid.yMin + maxHeight) + (labelDistance * 3) + YTopBuffer / adjustValue,
                                    0);
                                _parentForm.MapControl.Extents = ext;
                            }
                        }
                    }
                    else
                    {
                        if (chkAutoExpand.Checked)
                        {
                            _parentForm.MapControl.Extents = _layoutShapefile.Shape[rowIndex].Extents;
                        }
                    }
                }

                _parentForm.MapControl.Redraw();
            }

            if (isVisible)
            {
                _parentForm.MapLayersHandler.RefreshLayers();
            }
        }
        ...
        private void OnTabsSelectionChanged(object sender, EventArgs e)
        {
            ...
            switch (t.SelectedTab.Name)
            {
                    ...
                    if (global.MappingMode == fad3MappingMode.grid25Mode)
                    {
                        ...
                        lblProvideTitles.Visible = false;
                            ...
                            if (textFishingGround.Text.Length > 0
                                && _majorGrid.LayoutHelper.LayoutShapeFile != null
                ...
                case "tabExport":
                    ...
                    else if (global.MappingMode == fad3MappingMode.grid25Mode)
                    {
        ...
        private void PopulateExportSettings()
        {
            ...
            foreach (MapLayer ml in _parentForm.MapLayersHandler)
            {
                if (ml.Visible)
                {
                    if (ml.IsGrid25Layer)
        ...
        private void OnLayoutCheckChanged(object sender, EventArgs e)
        {
            lblLogoScaleFactor.Enabled = chkLayout.Checked;
            txtLogoScaleFactor.Enabled = chkLayout.Checked;
        }
    }
}