...
namespace GridMapper.views
{
    public partial class GraticuleForm : Form
    {
        ...
        public static GraticuleForm GetInstance(MainForm parent)
        {
            if (_instance == null) _instance = new GraticuleForm(parent);
            return _instance;
        }

        public GraticuleForm(MainForm parent)
        {
            InitializeComponent();
            _parentForm = parent;
        }
        ...
        private void ShowGraticule()
        {
            ...
            Graticule.MapScale = double.Parse(txtMapScale.Text);
            Graticule.TitleFontSize = int.Parse(txtTitleSize.Text);
            Graticule.AdjustScale();
            Graticule.ShowGraticule();
        }

        private void SetAllLabelsPositionToAboveParent()
        {
            foreach (var layer in global.MainForm.MapLayersHandler)
            {
        ...
        private void OnGraticuleForm_Load(object sender, EventArgs e)
        {
            ...
            Graticule.GraticuleExtentChanged += OnGraticuleExtentChanged;
            Graticule.MapRedrawNeeded += OnRedrawNeeded;
            txtMapScale.Text = global.MainForm.MapControl.CurrentScale.ToString();

            global.MainForm.MapControl.ExtentsChanged += MapControl_ExtentsChanged;
        }
        ...
        private void RefreshPreview()
        {
            ...
            var tempFileName = global.MainForm.SaveTempMapToImage();
