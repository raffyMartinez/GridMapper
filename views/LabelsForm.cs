...
namespace GridMapper.views
{
    public partial class LabelsForm : Form
    {
        ...
        private bool _inCategorizingLabels;
        ...
        public LabelCategory LabelCategory { get; set; }
        ...
        public LabelsForm(MapLayersHandler layersHandler, bool inCategorizingLabel = false)
        {
            InitializeComponent();
            _layersHandler = layersHandler;
            _layersHandler.OnVisibilityExpressionSet += OnVisibilityExpression;
            _mapLayer = _layersHandler.CurrentMapLayer;
            _shapeFile = _mapLayer.LayerObject as Shapefile;
            _inCategorizingLabels = inCategorizingLabel;
        }
        ...
        private void SetUpUI()
        {
            ...
            {
                //font tab
                _fontComboBox.Text = LabelCategory.FontName;
                txtFontSize.Text = LabelCategory.FontSize.ToString();
                txtFontTransparency.Text = LabelCategory.FontTransparency.ToString();
                chkItalic.Checked = LabelCategory.FontItalic;
                chkUnderline.Checked = LabelCategory.FontUnderline;
                chkBold.Checked = LabelCategory.FontBold;
                chkOutlineVisible.Checked = LabelCategory.FontOutlineVisible;
                rectFontColor.FillColor = FADColors.UintToColor(LabelCategory.FontColor);
                rectOutlineColor.FillColor = FADColors.UintToColor(LabelCategory.FontOutlineColor);
                txtFontOffsetX.Text = LabelCategory.OffsetX.ToString();
                txtFontOffsetY.Text = LabelCategory.OffsetY.ToString();
                switch (LabelCategory.Alignment)
                {
                ...
                //frame tab
                chkFrameVisible.Checked = LabelCategory.FrameVisible;
                comboFrameType.SelectedIndex = (int)LabelCategory.FrameType;
                txtFramePaddingX.Text = LabelCategory.FramePaddingX.ToString();
                txtFramePaddingY.Text = LabelCategory.FramePaddingY.ToString();
                txtFrameTransparency.Text = LabelCategory.FrameTransparency.ToString();
                txtFrameLineWidth.Text = LabelCategory.FrameOutlineWidth.ToString();
                rectFrameLineColor.FillColor = FADColors.UintToColor(LabelCategory.FrameOutlineColor);
                rectFrameFillColor.FillColor = FADColors.UintToColor(LabelCategory.FrameBackColor);
                switch (LabelCategory.InboxAlignment)
                {
                    case tkLabelAlignment.laTopLeft:
                ...
                //decorations
                chkHaloVisible.Checked = LabelCategory.HaloVisible;
                chkShadowVisible.Checked = LabelCategory.ShadowVisible;
                rectHaloColor.FillColor = FADColors.UintToColor(LabelCategory.HaloColor);
                rectShadowColor.FillColor = FADColors.UintToColor(LabelCategory.ShadowColor);
                txtHaloSize.Text = LabelCategory.HaloSize.ToString();
                txtShadowOffsetX.Text = LabelCategory.ShadowOffsetX.ToString();
                txtShadowOffsetY.Text = LabelCategory.ShadowOffsetY.ToString();
        ...
        private void OnFormLoad(object sender, EventArgs e)
        {
            ...
            SetUpUI();
            if (_inCategorizingLabels)
            {
                tabsLabelProperties.TabPages.Remove(tabSource);
                tabsLabelProperties.TabPages.Remove(tabVisibility);
            }
        }
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        LabelCategory.OffsetX = double.Parse(txtFontOffsetX.Text);
                        LabelCategory.OffsetY = double.Parse(txtFontOffsetY.Text);



                        if (optionTL.Checked)
                            LabelCategory.Alignment = tkLabelAlignment.laTopLeft;
                        else if (optionTC.Checked)
                            LabelCategory.Alignment = tkLabelAlignment.laTopCenter;
                        else if (optionTR.Checked)
                            LabelCategory.Alignment = tkLabelAlignment.laTopRight;
                        else if (optionCL.Checked)
                            LabelCategory.Alignment = tkLabelAlignment.laCenterLeft;
                        else if (optionCC.Checked)
                            LabelCategory.Alignment = tkLabelAlignment.laCenter;
                        else if (optionCR.Checked)
                            LabelCategory.Alignment = tkLabelAlignment.laCenterRight;
                        else if (optionBL.Checked)
                            LabelCategory.Alignment = tkLabelAlignment.laBottomLeft;
                        else if (optionBC.Checked)
                            LabelCategory.Alignment = tkLabelAlignment.laBottomCenter;
                        else if (optionBR.Checked)
                            LabelCategory.Alignment = tkLabelAlignment.laBottomRight;

                        LabelCategory.FontName = _fontComboBox.Text;
                        LabelCategory.FontTransparency = int.Parse(txtFrameTransparency.Text);
                        LabelCategory.FontSize = int.Parse(txtFontSize.Text);
                        LabelCategory.FontBold = chkBold.Checked;
                        LabelCategory.FontItalic = chkItalic.Checked;
                        LabelCategory.FontUnderline = chkUnderline.Checked;
                        LabelCategory.FontOutlineVisible = chkOutlineVisible.Checked;
                        LabelCategory.FontOutlineColor = FADColors.ColorToUInteger(rectOutlineColor.FillColor);
                        LabelCategory.FontColor = FADColors.ColorToUInteger(rectFontColor.FillColor);

                        LabelCategory.FrameVisible = chkFrameVisible.Checked;
                        LabelCategory.FrameTransparency = int.Parse(txtFrameTransparency.Text);
                        LabelCategory.FrameOutlineWidth = int.Parse(txtFrameLineWidth.Text);
                        LabelCategory.FrameType = ((KeyValuePair<tkLabelFrameType, string>)comboFrameType.SelectedItem).Key;
                        LabelCategory.FrameBackColor = FADColors.ColorToUInteger(rectFrameFillColor.FillColor);
                        ...
                        if (optionFrameTL.Checked)
                            ...
                            LabelCategory.InboxAlignment = tkLabelAlignment.laTopCenter;
                        else if (optionFrameTR.Checked)
                            LabelCategory.InboxAlignment = tkLabelAlignment.laTopRight;
                        else if (optionFrameCL.Checked)
                            LabelCategory.InboxAlignment = tkLabelAlignment.laCenterLeft;
                        else if (optionFrameCC.Checked)
                            LabelCategory.InboxAlignment = tkLabelAlignment.laCenter;
                        else if (optionFrameCR.Checked)
                            LabelCategory.InboxAlignment = tkLabelAlignment.laCenterRight;
                        else if (optionFrameBL.Checked)
                            LabelCategory.InboxAlignment = tkLabelAlignment.laBottomLeft;
                        else if (optionFrameBC.Checked)
                            LabelCategory.InboxAlignment = tkLabelAlignment.laBottomCenter;
                        else if (optionFrameBR.Checked)
                            LabelCategory.InboxAlignment = tkLabelAlignment.laBottomRight;

                        LabelCategory.HaloVisible = chkHaloVisible.Checked;
                        LabelCategory.HaloSize = int.Parse(txtHaloSize.Text);
                        LabelCategory.HaloColor = FADColors.ColorToUInteger(rectHaloColor.FillColor);

                        LabelCategory.ShadowVisible = chkShadowVisible.Checked;
                        LabelCategory.ShadowColor = FADColors.ColorToUInteger(rectShadowColor.FillColor);
                        LabelCategory.ShadowOffsetX = int.Parse(txtShadowOffsetX.Text);
                        LabelCategory.ShadowOffsetY = int.Parse(txtShadowOffsetY.Text);
                    }
                }
            }
        }
