...

{
    ...
    public class MapLayersHandler : IDisposable, IEnumerable<MapLayer>
    {
        ...
        public string VisibilityExpressionText { get; private set; }
        ...
        public delegate void LayerAddError(MapLayersHandler s, LayerEventArg e);
        public event LayerAddError ErrorAddingLayer;
        ...
        public int AddLayer(Shapefile sf, string layerName = "", bool isVisible = true, bool uniqueLayer = false, fad3MappingMode mappingMode = fad3MappingMode.defaultMode)
        {
            ...
            try
            {
                ...
                h = _axmap.AddLayer(sf, isVisible);
                if (h >= 0)
                {
                    ...
                    _currentMapLayer.MappingMode = global.MappingMode;
            ...
            {
                ErrorMessage = $"{comerr.Message}";
                if (ErrorAddingLayer != null)
                {
                    ErrorAddingLayer(this, new LayerEventArg(layerName) { ErrorMessage = comerr.Message });
                }
            }
            catch (Exception ex)
            {
