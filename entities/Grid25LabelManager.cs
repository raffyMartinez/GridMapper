...
namespace GridMapper.entities
{
    public class Grid25LabelManager : IDisposable
    {
        ...
        //refers to the table field for Label

        public string MapFootnote { get; set; }
        ...
        public bool LabelGrid(Shapefile sfLabelPath,
            ...
            string mapTitle,
            double titleDistanceFactor,
            bool printFrontAndReverse = false,
            bool reverseSide = false,
            bool showTitle = false,
            bool showZone = false)
        {
            TitleDistanceFactor = titleDistanceFactor;
            bool success = false;
        ...
        private void SetMapTitle(Extents minorGridExtent, bool showZone = true, bool shoSizeStatment = true, bool showCitation = true)
        {
            ...
            if (shp.Create(ShpfileType.SHP_POINT))
            {
                ...
                if (shp.AddPoint(minorGridExtent.xMin, (minorGridExtent.yMax + 4000 + _labelDistance) * TitleDistanceFactor) >= 0)
                {
                    var iShp = _shapeFileGrid25Labels.EditAddShape(shp);
                    if (iShp >= 0)
                    {
            ...
            if (showZone)
            {
                ...
                if (shp.Create(ShpfileType.SHP_POINT))
                {
                    yPos = (2 - TitleDistanceFactor) * (minorGridExtent.yMin - 3000 - _labelDistance);
                    //if (shp.AddPoint(minorGridExtent.xMin, TitleDistanceFactor * -1 * (minorGridExtent.yMin - 3000 - _labelDistance)) >= 0
            ...
            if (showCitation)
            {
                ...
                if (shp.Create(ShpfileType.SHP_POINT))
                {
                    yPos = (2 - TitleDistanceFactor) * (minorGridExtent.yMin - 6000 - _labelDistance);
                    if (shp.AddPoint(minorGridExtent.xMin, yPos) >= 0)
                    {
                        var iShp = _shapeFileGrid25Labels.EditAddShape(shp);
                        if (iShp >= 0)
        ...
        public void SetupLabelProperties()
        {
            ...
            _shapeFileGrid25Labels.Labels.AddCategory("MF");
            _shapeFileGrid25Labels.Labels.AddCategory("CIT");
