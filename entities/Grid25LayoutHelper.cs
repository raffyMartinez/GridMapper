        ...
        public bool SaveLayoutTemplate(string fileName)
        {
            ...
            if (ValidLayoutTemplateShapefile())
            {
                ...
                if (_sfLayout.SaveAs(fileName))
                {
                    ...
                    {
                        writer.WriteLine($"Fishing ground:{ global.MainForm.Grid25MajorGrid.LayoutHelper.FishingGround}");
                        writer.WriteLine($"Rows:{global.MainForm.Grid25MajorGrid.LayoutHelper.Rows}");
                        writer.WriteLine($"Columns:{global.MainForm.Grid25MajorGrid.LayoutHelper.Columns}");
                        writer.WriteLine($"Overlap:{global.MainForm.Grid25MajorGrid.LayoutHelper.Overlap}");
                        string selectedMajorGrids = "SelectedMajorGrids:";
                        foreach (int item in global.MainForm.Grid25MajorGrid.SelectedShapeGridNumbers)
                        {
