...
namespace GridMapper.entities
{
    ...
    public class Graticule : IDisposable
    {
        ...
        public Graticule(AxMap mapControl, MapLayersHandler layersHandler)
        {
            ...
            if (_mapLayersHandler.MapControl.TileProvider == tkTileProvider.ProviderNone)
            {
                _mapLayersHandler.MapControl.ZoomBehavior = tkZoomBehavior.zbDefault;
            }
            else
            {
                _mapLayersHandler.MapControl.ZoomBehavior = tkZoomBehavior.zbUseTileLevels;
            }
        }
        ...
        {
            if (Math.Abs(((int)_mapLayersHandler.MapControl.CurrentScale) - ((int)MapScale)) > 1)
            {
                _mapLayersHandler.MapControl.CurrentScale = MapScale;
            }
            ...
            for (int n = 0; n < _sfGraticule.Labels.NumCategories; n++)
            {
                ...
                switch (_sfGraticule.Labels.Category[n].Name)
                {
                    ...
                    case "Title":
                        ...
                        _sfGraticule.Labels.Category[n].FontSize = TitleFontSize;
